...
export function simpleContexts(): [() => { [key in ContextKey]: Context<key> }, () => void, (context: ContextKey, callback: <Key extends ContextKey>(context: Key, value: Context<Key>) => void) => void] {
    ...
    Dispatcher.addListener("COLORWAYS_CONTEXT_UPDATED", <Key extends ContextKey>(params) => {
        const { c, value } = params as { c: Key, value: Context<Key>; };
        val[c] = value;
        if (events[c]) {
            events[c].forEach(callback => callback(c, value));
    ...
    return [
        () => val,
        () => {
            Dispatcher.removeListener("COLORWAYS_CONTEXT_UPDATED", <Key extends ContextKey>(params) => {
                const { c, value } = params as { c: Key, value: Context<Key>; };
                val[c] = value;
                if (events[c]) {
                    events[c].forEach(callback => callback(c, value));
